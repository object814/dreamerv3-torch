...
class MergedEpisodes:
    """Dict-like view that merges multiple episode dicts for sample_episodes().

    sample_episodes() calls .values() to compute sampling probabilities and
    to draw episodes. This class provides a unified view over the current task's
    episodes (which grow during training via simulate()) and static ER buffer
    episodes. Changes to the underlying dicts are reflected immediately.
    """

    def __init__(self, *episode_dicts):
        self._dicts = episode_dicts
...
def reservoir_sample_episodes(directory, budget, seed=0):
    """Load episodes from a directory using reservoir sampling.

    Randomly shuffles episode files and loads until the transition budget is met.
    This is equivalent to reservoir sampling when all files are available upfront.

    Args:
        directory: Path to directory containing .npz episode files.
        budget: Maximum number of transitions to load.
        seed: Random seed for reproducible sampling.

    Returns:
        OrderedDict of episode_id -> episode_data (numpy arrays).
    """
    directory = pathlib.Path(directory).expanduser()
    if not directory.exists():
        return collections.OrderedDict()

    all_files = sorted(directory.glob("*.npz"))
    if not all_files:
        return collections.OrderedDict()
    ...
    for idx in indices:
        ...
        try:
            ...
            print(f"Could not load ER episode {filename}: {e}")
            continue
        ...
        key = f"er_{os.path.splitext(os.path.basename(filename))[0]}"
...
def main(args, remaining_args):
    ...
    for task_idx in range(num_tasks):
        ...
        # Create eval envs for ALL tasks seen so far
        all_eval_envs = {}
        all_eval_dirs = {}
        all_eval_caches = {}
        ...
        print(f">>> SEQUENTIAL ER: Created eval envs for {task_idx + 1} task(s)")
        ...
        if not config.offline_traindir:
            ...
            print(f">>> SEQUENTIAL ER: Prefill dataset ({prefill} steps).")
            if prefill > 0:
                ...
                print(f">>> SEQUENTIAL ER: Logger step after prefill: {logger.step}")
        ...
        if er_buffer:
            ...
            train_dataset = make_dataset(merged_view, config)
            print(f">>> SEQUENTIAL ER: Dataset merges current task + ER buffer "
                  f"({len(train_eps)} current + {len(er_buffer)} ER episodes)")
        else:
    ...
    print("=" * 60)
    print(">>> SEQUENTIAL ER: All tasks completed successfully!")
    print(f">>> SEQUENTIAL ER: Final global step: {global_env_step}")
    print("=" * 60)
