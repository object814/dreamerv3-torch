...
# To keep things simple and consistent, we use the dreamer_sequential.py script
# with a single task and config. This allows us to reuse the same training
# loop and logging code, and makes it easy to compare with the multi-task version.

# Configuration
WANDB_ENTITY="haoyu-a2i"
WANDB_PROJECT="Metaworld_Dreamerv3_Single"
RUN_NAME="mw_single_boxclose_$(date +%m%d)"
LOGDIR="../logdir/single/${RUN_NAME}"

# Task environment name
TASKS=(
    "metaworld_box-close-v3"
)
